...
const useAlphaTimingSystem = (track: string, driverName: string) => {
  const { sendJsonMessage, lastJsonMessage, readyState } = useWebSocket(
    "wss://ws-eu.pusher.com/app/3aaffebc8193ea83cb2f?protocol=7&client=js&version=3.1.0&flash=false",
  );
  ...
  const getSelectedDriverDataFromJson = async () => {
    ...
    const response = await fetch(
      `https://live.alphatiming.co.uk/${track}/competitors/${driverIdentifier.id}.json?session_id=${sessionId.current}`,
    );
    ...
    const quickestLaps = driverSetup["Competitors"][0]["Laps"]
      .sort((lapA: any, lapB: any) => lapA["LapTime"] - lapB["LapTime"])
      .slice(0, 3)
      .map((lap: any) => ({ lap: lap["LapNumber"], time: lap["LapTime"] }));
  ...
  const setSelectedDriverDataFromJson = (selectedDriver: any) => {
    if (
      topThreeLaps.findIndex(
        (topLap) => topLap.lap === selectedDriver["BestLapNumber"],
      ) === -1
    ) {
      setTopThreeLaps(
        [
    ...
    if (selectedDriver["Laps"]) {
      ...
      if (lap["LapTime"]) {
        ...
        if (lap["LapTime"] && lap["LapTime"] < topThreeLaps[2].time) {
          ...
          if (
            previousBestLap < Infinity &&
            lap["LapTime"] === sortedTopThreeLaps[0].time
          )
  ...
  const getCompetitionData = async () => {
    ...
    const response = await fetch(
      `https://live.alphatiming.co.uk/${track}.json`,
    );

    const raceSetup = await response.json();

    setCompetitors(
      raceSetup["Competitors"].map((competitor: any) => ({
        ...competitor,
  ...
  const getDriverData = async (subscribeToDataChannel: boolean = true) => {
    ...
    const driver = competition?.find(
      (competitor: any) => competitor["CompetitorName"] === driverName,
    );
  ...
  const registerCompetitor = async (competitor: any) => {
    ...
    if (
      ...
      competitor["CompetitorName"] === driverName
    ) {
    ...
    const index = socketCompetitors.findIndex(
      (competitor) => competitor.id === competitor["CompetitorId"],
    );
    if (index > -1) {
      ...
      const stillUndiscoveredCompetitors = socketCompetitors.some(
        (localCompetitor) => localCompetitor.appearances < 3,
      );
      ...
      setSocketCompetitors([
        ...socketCompetitors,
        {
          ...
          name: competitor["CompetitorName"] || "",
          registered:
            competitor["CompetitorId"] && competitor["CompetitorName"], // TODO: make sure this returns true or flase depending on the combination of values
          appearances: 1,
        },
      ]);
    }
    ...
    if (event === "new_session") {
      ...
      if (data["Competitors"]) {
        ...
        for (let i = 0; i < competitors.length; i++) {
          ...
          if (
            competitor["BestLaptime"] &&
            competitor["BestLaptime"] < bestLapTime
          )
            setBestLapTime(competitor["BestLaptime"]);

          if (!registered) registerCompetitor(competitor);

          if (
            competitor["CompetitorId"] !== driverIdentifier.id &&
            competitor["Laps"]
          ) {
            if (
              competitor["Laps"][0]["Position"] === position + 1 &&
              competitor["Laps"][0]["Gap"]
            ) {
